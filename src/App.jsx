...
/* ═══════════════════ NAV ═══════════════════ */
function Nav({isPub,go,userName,title,canGoBack=false,onBack}){return(<nav className="nav"><div style={{display:"flex",alignItems:"center",gap:12}}><div className="logo">ف</div><div style={{textAlign:"right"}}><p style={{fontSize:"1.2rem",fontWeight:900,color:"#fff",lineHeight:1}}>فهمني</p><p style={{fontSize:".67rem",color:"#64748b",marginTop:3}}>{isPub?"ذاكر بذكاء، وافهم أسرع":title||""}</p></div></div><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>{isPub?<>{canGoBack&&<button className="btn btn-g" onClick={onBack}>← رجوع</button>}<button className="btn btn-g" onClick={()=>go("login")}>تسجيل الدخول</button><button className="btn btn-p" onClick={()=>go("signup")}>ابدأ مجانًا ←</button></>:<>{canGoBack&&<button className="btn btn-g" style={{fontSize:".78rem",padding:"9px 14px"}} onClick={onBack}>← رجوع</button>}<button className="btn btn-g" style={{fontSize:".78rem",padding:"9px 14px"}} onClick={()=>go("landing")}>🌐 العودة للموقع</button><button className="btn btn-g" style={{fontSize:".78rem",padding:"9px 14px"}} onClick={()=>go("dashboard")}>↩ لوحة الطالب</button><div style={{padding:"7px 13px",borderRadius:11,background:"rgba(249,115,22,.1)",border:"1px solid rgba(249,115,22,.2)",fontSize:".78rem",fontWeight:700,color:"#fdba74"}}>{userName}</div></>}</div></nav>);}
...
function Landing({go}){return(<div style={{display:"grid",gap:16}}><div className="gl au" style={{padding:"46px 36px"}}><span className="badge b-o" style={{marginBottom:14}}>✦ تدريب ذكي · AI منضبط · خريطة مسار</span><h1 style={{fontSize:"2.4rem",fontWeight:900,lineHeight:1.22,color:"#fff",maxWidth:560}}>بعض الأسئلة تحتاج أحد<br/><span style={{background:"linear-gradient(135deg,#f97316,#fb923c,#22d3ee)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>يفهّمك إياها صح</span></h1><p style={{marginTop:12,fontSize:".92rem",lineHeight:1.9,color:"#64748b",maxWidth:560}}>فهمني منصتك الكاملة للقدرات — تدريب واضح، شرح AI مضبوط، ومزايا مدفوعة متوازنة تغطي قيمة المحتوى وتكلفة الذكاء الاصطناعي بدون مبالغة.</p><div style={{display:"flex",gap:11,marginTop:22,flexWrap:"wrap"}}><button className="btn btn-p" style={{fontSize:".93rem",padding:"13px 26px"}} onClick={()=>go("signup")}>ابدأ مجانًا ← 5 أسئلة</button><button className="btn btn-g" onClick={()=>go("pricing")}>عرض الباقات</button></div></div>
...
<div className="gl" style={{padding:"20px 26px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}><p style={{fontWeight:700,color:"#e2e8f0"}}>جرّب 5 أسئلة مجانًا، ثم افتح كامل المنصة من 49 ريال</p><div style={{display:"flex",gap:9}}><button className="btn btn-g" onClick={()=>go("login")}>عندي حساب</button><button className="btn btn-p" onClick={()=>go("signup")}>أنا مستخدم جديد ←</button></div></div>
</div>);}
...
function Paywall({trial,subscribe,back,go}){return(<div style={{display:"grid",gap:14}}><div style={{padding:"40px 34px",borderRadius:22,background:"linear-gradient(135deg,rgba(249,115,22,.13),rgba(34,211,238,.07))",border:"1px solid rgba(249,115,22,.22)"}}><span className="badge b-o" style={{marginBottom:12}}>انتهت التجربة المجانية</span><h1 style={{fontSize:"1.9rem",fontWeight:900,color:"#fff"}}>جرّبت المنصة — والآن<span style={{display:"block",marginTop:7,color:"#fdba74"}}>اختر الخطة الأنسب لك 🎯</span></h1></div><div style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:14}}><div className="gl" style={{padding:"24px"}}><p style={{fontSize:".68rem",color:"#f97316",fontWeight:700,letterSpacing:".08em",marginBottom:13}}>كل ما تحتاجه للتقدم الحقيقي</p>{["بنك الأسئلة الكامل","شرح AI منظم حسب الخطة","وضع المحاكاة الكامل ⚡","مراجعة الأخطاء الذكية","خطة مذاكرة أوضح","تتبع التقدم بشكل احترافي"].map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{color:"#4ade80",fontWeight:900}}>✓</span><span style={{fontSize:".84rem",color:"#e2e8f0"}}>{item}</span></div>))}</div><div style={{display:"flex",flexDirection:"column",gap:11}}><div className="gl" style={{padding:"20px",textAlign:"center",background:"linear-gradient(145deg,rgba(249,115,22,.1),rgba(5,9,26,.9))",border:"1px solid rgba(249,115,22,.3)"}}><p style={{fontSize:".68rem",color:"#f97316",fontWeight:700,marginBottom:8}}>الخطة المتقدمة</p><p style={{fontSize:"3rem",fontWeight:900,color:"#fff",lineHeight:1}}>49</p><p style={{fontSize:".8rem",color:"#fdba74",fontWeight:700,marginBottom:8}}>ريال / شهر</p><p style={{fontSize:".72rem",color:"#94a3b8",marginBottom:16}}>شرح AI حتى 25 مرة يوميًا + كل الأساسيات</p><button className="btn btn-p" style={{width:"100%",justifyContent:"center",padding:"12px"}} onClick={subscribe}>اشترك الآن ←</button></div><div className="gl" style={{padding:"18px",textAlign:"center",border:"1px solid rgba(167,139,250,.35)",background:"linear-gradient(145deg,rgba(167,139,250,.08),rgba(5,9,26,.92))"}}><p style={{fontSize:".68rem",color:"#a78bfa",fontWeight:700,marginBottom:8}}>الخطة الاحترافية</p><p style={{fontSize:"2.2rem",fontWeight:900,color:"#fff",lineHeight:1}}>99</p><p style={{fontSize:".78rem",color:"#c4b5fd",fontWeight:700,marginBottom:8}}>ريال / 3 أشهر</p><p style={{fontSize:".72rem",color:"#94a3b8"}}>ذكاء أعلى، محاكاة أكثر، وتقارير أدق</p></div><button className="btn btn-g" style={{justifyContent:"center",fontSize:".82rem"}} onClick={()=>go("pricing")}>عرض كل الباقات</button><button className="btn btn-g" style={{justifyContent:"center",fontSize:".82rem"}} onClick={back}>العودة</button></div></div></div>);}
...
export default function Fahmni(){
  ...
  const[trial,setTrial]=useState({isSubscribed:false,used:0,limit:5});
  ...
  const go=p=>{setPage(cur=>cur===p?cur:p);setHistory(prev=>prev[prev.length-1]===p?prev:[...prev,p]);window.scrollTo({top:0,behavior:"smooth"});};
  const goBack=()=>{setHistory(prev=>{if(prev.length<=1)return prev;const next=prev.slice(0,-1);const target=next[next.length-1]||"landing";setPage(target);window.scrollTo({top:0,behavior:"smooth"});return next;});};
  ...
  const R=()=>{switch(page){
    ...
    case"paywall":return <Paywall trial={trial} go={go} subscribe={()=>{setTrial(p=>({...p,isSubscribed:true}));go("dashboard");}} back={goBack}/>;
  ...
  return(
    <div className="app">
      <GS/><Bg/>
      <div className="wrap">
        <Nav isPub={PUB.includes(page)} go={go} userName={user.name} title={TITLES[page]||""} canGoBack={history.length>1} onBack={goBack}/>
        <R/>
      </div>
    </div>
  );
