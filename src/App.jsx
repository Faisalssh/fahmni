...
function Nav({isPub,go,userName,title,onLogout}){
  return(
    <nav className="nav">
      <div style={{display:"flex",alignItems:"center",gap:12,maxWidth:1020,margin:"0 auto",width:"100%",padding:"0 18px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,flex:1}}>
          <div className="logo" onClick={()=>go(isPub?"landing":"dashboard")} style={{cursor:"pointer"}}>ف</div>
          <div style={{textAlign:"right"}}>
            <p style={{fontSize:"1.1rem",fontWeight:900,color:"#fff",lineHeight:1}}>فهمني</p>
            <p style={{fontSize:".63rem",color:"#64748b",marginTop:2}}>{isPub?"ذاكر بذكاء، وافهم أسرع":title||""}</p>
          </div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {isPub?(
            <>
              <button className="btn btn-g" style={{fontSize:".8rem",padding:"9px 16px"}} onClick={()=>go("login")}>تسجيل الدخول</button>
              <button className="btn btn-p" style={{fontSize:".8rem"}} onClick={()=>go("signup")}>ابدأ مجانًا ←</button>
            </>
          ):(
            <>
              <button className="btn btn-g" style={{fontSize:".75rem",padding:"7px 13px"}} onClick={()=>go("landing")}>🏠 الرئيسية</button>
              <button className="btn btn-g" style={{fontSize:".75rem",padding:"7px 13px"}} onClick={()=>go("dashboard")}>↩ لوحتي</button>
              <div style={{padding:"6px 12px",borderRadius:10,background:"rgba(249,115,22,.1)",border:"1px solid rgba(249,115,22,.2)",fontSize:".75rem",fontWeight:700,color:"#fdba74"}}>{userName}</div>
              {onLogout&&<button className="btn btn-g" style={{fontSize:".72rem",padding:"6px 11px",color:"#64748b"}} onClick={onLogout}>خروج</button>}
            </>
          )}
        </div>
      </div>
...
function Landing({go}){
  ...
  return(
    <div style={{display:"grid",gap:0}}>
      ...
      {/* ── Hero ── */}
      <div style={{padding:"64px 0 48px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap",animation:"fadeUp .5s .2s both"}}>
      ...
      {/* ── CTA النهائي ── */}
      <div style={{padding:"48px 32px",borderRadius:24,
...
function Auth({mode,go,onLogin}){
  ...
  const guestLogin=()=>{
    ...
    onLogin({token:"guest",userId:"guest",name:guestName,email:"guest@fahmni.sa",isGuest:true});
  };
...
function LegalPage({title,badge,badgeClass,sections,go}){
  return(
    <div style={{display:"grid",gap:14,maxWidth:720,margin:"0 auto",width:"100%"}}>
      <div className="gl au" style={{padding:"36px 32px"}}>
...
export default function Fahmni(){
  ...
  const PUB=["landing","login","signup","privacy","terms","contact"];
